...
def get_default_ai_provider():
    """
    Determines default AI provider based on environment.
    Internal Environment (DB Connected) -> OpenAI (vLLM)
    External Environment -> Gemini
    """
    try:
        if FDALabelDBService.check_connectivity():
            return 'openai'
    except Exception:
        # Fallback if check fails (e.g. outside app context)
        pass
    return 'gemini'
...
class User(UserMixin, db.Model):
    ...
    # AI Preferences
    ai_provider = db.Column(db.String(20), default=get_default_ai_provider) # Dynamic default based on env
    custom_gemini_key = db.Column(db.String(255), nullable=True)
    openai_api_key = db.Column(db.String(255), nullable=True)
    openai_base_url = db.Column(db.String(255), nullable=True) # For vLLM/Local
...
class LabelAnnotation(db.Model):
    ...
    # Target Location
    section_id = db.Column(db.String(100), nullable=False) # HTML ID or data-attribute
    ...
    # Content
    annotation_type = db.Column(db.String(20), nullable=False) # 'highlight' or 'comment'
    color = db.Column(db.String(20)) # e.g. 'yellow', '#ff0000', 'none'
...
class ComparisonSummary(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    set_ids_hash = db.Column(db.String(64), unique=True, nullable=False) # Hash of sorted set_ids
    set_ids = db.Column(db.Text, nullable=False) # JSON string of list of set_ids
...
class PgxSynonym(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    term = db.Column(db.String(255), unique=True, nullable=False, index=True) # Lowercase search term
    normalized_name = db.Column(db.String(255), nullable=False) # Canonical display name

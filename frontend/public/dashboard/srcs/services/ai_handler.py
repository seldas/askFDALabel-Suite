...
class AIClientFactory:
    ...
    def get_client(user=None):
        ...
        if user.ai_provider == 'openai':
            client = OpenAI(
                api_key=os.getenv("INTERNAL_VLLM_API_KEY", os.getenv("LLM_KEY")),
                base_url=os.getenv("INTERNAL_VLLM_BASE_URL", os.getenv("LLM_URL"))
            )
            return "openai", client, os.getenv("INTERNAL_VLLM_MODEL_NAME", os.getenv("LLM_MODEL", "gpt-4o"))

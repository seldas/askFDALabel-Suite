(function() {
    ...
    function showTooltip(e, text, linkElement) {
        ...
        // Fetch info
        fetch(`https://ncshpcgpu01:8845/api/snippet-preview?drug_name=${encodeURIComponent(text)}`)
            .then(res => res.json())
            .then(data => {
                if (data.found) {
                    ...
                    let baseUrl = '';
                    if (currentOption === 1 ) {
                        baseUrl = `http://ncshpcgpu01:8844/label/${data.set_id}`;
                    } else if (currentOption === 2 ) {
                        baseUrl = `https://fdalabel.fda.gov:8443/fdalabel/services/spl/set-ids/${data.set_id}/spl-doc`;
                    } else {
                        baseUrl = `http://ncshpcgpu01:8844/search?drug_name=${encodeURIComponent(text)}`;
                    }
    ...
    function highlightText(node) {
        ...
        matches.forEach(match => {
            ...
            if (currentOption === 1 && link.dataset.setId) {
                baseUrl = `http://ncshpcgpu01:8844/label/${link.dataset.setId}`;
                ...
                baseUrl = `http://ncshpcgpu01:8844/search?drug_name=${encodeURIComponent(match.original)}`;
            }
            link.href = baseUrl;
            link.target = '_blank';
    ...
    document.addEventListener('mouseup', (e) => {
        ...
        if (selectedText.length > 1 && selectedText.length < 50) {
            ...
            popup.onmousedown = (pe) => {
                ...
                let baseUrl = `http://ncshpcgpu01:8844/search?drug_name=${encodeURIComponent(selectedText)}`;
